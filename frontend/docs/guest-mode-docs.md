...
- **Code** (from `sendQuery`):
  ...
  if (!q || !grecaptcha.getResponse() || isRunning || ws.readyState !== WebSocket.OPEN) {
      return;
  }
  ws.send(JSON.stringify({
      sessionId: sessionId,
...
- **Code** (from `ws.onmessage`):
  ...
  ws.onmessage = (event) => {
      ...
      if (msg.status === 'connected') {
          ...
          appendOutput(`"${msg.message.split('"')[1]}" (type: ${selectedType}) queued`, 'info');
...
3. **Send Initial Message**:
   ```json
   {"sessionId":null,"query":null,"type":"general","recaptchaResponse":"test_token"}
   ```

4. **Send Query**:
   ```json
   {"sessionId":"session_123","query":"solar energy labs","type":"general","recaptchaResponse":"test_token"}
   ```
