...
import { ArrowLeft, Upload, User, MapPin, Building, Award, Link, Plus, X, Calendar, Phone, Globe, Mail } from "lucide-react";
...
export default function NewProfilePage() {
    ...
    const checkProfileCount = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.length >= 2) {
            toast.error("You have reached the maximum limit of 2 profiles");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("token") || sessionStorage.getItem("token") || "";
      const response = await fetch("/api/profiles", { method: "POST", body: data, headers: { Authorization: `Bearer ${token}` } });
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="hidden sm:block">
                <h1 className="font-semibold">Create Profile</h1>
                <p className={clsx("text-xs", isDark ? "text-gray-500" : "text-gray-500")}>Build your identity</p>
              </div>
            </div>
            <button onClick={handleSubmit} disabled={loading} className={clsx("px-4 sm:px-5 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2", loading ? "opacity-50 cursor-not-allowed" : "bg-[#00FB75] text-black hover:bg-green-400")}>
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-6 pb-24 sm:pb-8 space-y-4 sm:space-y-6">
          <div className={sectionClass}>
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <div onClick={() => document.getElementById("profile-image")?.click()} className={clsx("w-20 h-20 sm:w-24 sm:h-24 rounded-full border-2 border-dashed cursor-pointer overflow-hidden transition-all hover:scale-105", isDark ? "border-gray-600 hover:border-gray-400 bg-gray-800" : "border-gray-300 hover:border-gray-400 bg-gray-100")}>
              ...
              <input id="profile-image" type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
              <div>
                <p className={clsx("text-xs mb-2", isDark ? "text-gray-500" : "text-gray-500")}>Recommended: 400×400px, JPG/PNG</p>
                <button type="button" onClick={() => document.getElementById("profile-image")?.click()} className={clsx("px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2", isDark ? "bg-gray-800 hover:bg-gray-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-700")}>
          ...
          <div className={sectionClass}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><label className={labelClass}>Gender</label><select name="gender" value={formData.gender} onChange={handleInputChange} className={inputClass}>{genderOptions.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}</select></div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={loading} className={clsx("flex-1 py-2.5 sm:py-3 rounded-lg text-sm font-medium transition-colors", loading ? "opacity-50 cursor-not-allowed bg-[#00FB75] text-black" : "bg-[#00FB75] text-black hover:bg-green-400")}>{loading ? "Creating..." : "Create Profile"}</button>
          </div>
        </form>
      </div>
    </div>
