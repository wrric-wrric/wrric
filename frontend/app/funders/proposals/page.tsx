...
export default function FunderProposalsPage() {
  ...
  const fetchProposals = async () => {
    try {
      ...
      const response = await fetch("/api/funders/proposals", {
        headers: {
  ...
  const updateProposalStatus = async (proposalId: string, status: string) => {
    try {
      ...
      toast.success(`Proposal ${status === 'accepted' ? 'accepted' : 'declined'}!`);
    } catch (error) {
  ...
  const filteredProposals = proposals.filter(proposal => {
    const matchesSearch = 
      ...
      proposal.entity.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      proposal.summary.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesStatus = statusFilter === "all" || proposal.status === statusFilter;
    const matchesStage = stageFilter === "all" || proposal.stage === stageFilter;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm opacity-70 mt-1">
                Review and manage funding proposals from entrepreneurs
              </p>
            </div>
            <div className="flex items-center gap-4">
              <span className={`px-3 py-1 rounded-full text-sm ${isDark ? "bg-gray-800" : "bg-gray-200"}`}>
            ...
            {filteredProposals.length === 0 ? (
              <div className={`text-center py-16 rounded-xl ${isDark ? "bg-gray-900" : "bg-white shadow-sm"}`}>
                ...
                <p className="text-gray-500 mb-6 max-w-md mx-auto">
                  {proposals.length === 0 
                    ? "You haven't received any funding proposals yet." 
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className={`p-4 rounded-lg ${isDark ? "bg-gray-800" : "bg-gray-50"}`}>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="font-medium">{proposal.entity.name}</span>
                        </div>
                        <div className="text-sm opacity-70">
                          {proposal.entity.university}
                          {proposal.entity.location.city && ` • ${proposal.entity.location.city}`}
                          {proposal.entity.location.country && `, ${proposal.entity.location.country}`}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-2 lg:w-48">
                      ...
                      <button
                        onClick={() => router.push(`/entities/${proposal.entity.id}`)}
                      ...
                      <button
                        onClick={() => router.push(`/messages?to=${proposal.entity.id}`)}
